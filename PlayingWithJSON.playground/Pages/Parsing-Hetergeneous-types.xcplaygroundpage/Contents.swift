...
// ex: postcode on occassion is an Int or a String
...
struct Location: Decodable {
    let city: String
    let country: String
    let postcode: Postcode // Could be a string
}
...
// create a custom object to represent
enum Postcode: Decodable {
    // handle the two cases in the JSON "postcode" poperty where postcode can be either Int or a String
    
    // we will also use associative values to capture the value of postcode
    // e.g. if its an Int we will capture the Int value
    ...
    init(from decoder: Decoder) throws {
        ...
        // if its not an int or a string then we will throw an error
        if let intValue = try? decoder.singleValueContainer().decode(Int.self) { // this returns the data stored in the decoder
            self = .int(intValue)
            return
        }
        
        if let stringValue = try? decoder.singleValueContainer().decode(String.self) { // this returns the data stored in the decoder
...
do {
    let dictionary = try JSONDecoder().decode(ResultsWrapper.self, from: data)
    ...
    case .int(let intValue):
        print("postcode is int: \(intValue)")
    case .string(let stringValue):
         print("postcode is int: \(stringValue)")
    }
